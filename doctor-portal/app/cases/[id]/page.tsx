...
import PatientEvidencePanel from "@/components/PatientEvidencePanel";
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          {/* === Clinician Explainability Layers === */}
          {caseData.explainability && (
            <div className="space-y-4">
              <div className="flex items-center gap-2 pt-2">
                <Layers className="w-4 h-4 text-who-blue" />
                <h2 className="text-sm font-heading font-bold text-gray-900 uppercase tracking-wide">
                  Explainability Layers
                </h2>
                <div className="flex-1 h-px bg-gray-200" />
              </div>

              {/* Layer 1: Language & Communication Banner */}
              {caseData.explainability.language_banner && (
                <LanguageBanner banner={caseData.explainability.language_banner} />
              )}

              {/* Layer 2: Patient Evidence (original utterances + translations) */}
              {caseData.explainability.patient_evidence && caseData.explainability.patient_evidence.length > 0 && (
                <PatientEvidencePanel
                  evidence={caseData.explainability.patient_evidence}
                  patientLanguage={caseData.explainability.language_banner?.patient_language || "en"}
                />
              )}

              {/* Layer 3: Structured Extraction with source attribution */}
              {caseData.explainability.extraction && (
                <ExtractionPanel extraction={caseData.explainability.extraction} />
              )}

              {/* Layer 4: Safety & Triage */}
              {caseData.explainability.safety && (
                <SafetyPanel
                  safety={caseData.explainability.safety}
                  triageBreakdown={caseData.triageBreakdown}
                />
              )}

              {/* Layer 5: Unresolved Ambiguities */}
              {caseData.explainability.ambiguity && (
                <AmbiguityPanel ambiguity={caseData.explainability.ambiguity} />
              )}
            </div>
          )}
