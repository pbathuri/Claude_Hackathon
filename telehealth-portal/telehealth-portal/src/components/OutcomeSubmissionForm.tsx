...
import { type PatientCase, type TierRule, type OutcomeSubmission, RecommendedAction, CountryTier } from "../types";
...
export default function OutcomeSubmissionForm({ patientCase: c, tierRule, doctorId }: Props) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      await submitOutcome(c.caseId, doctorId, outcome);
      setSubmitted(true);
    } catch (err) {
      setErrors({
        generalNotes:
          err instanceof Error ? err.message : "Failed to submit outcome to the server.",
      });
    } finally {
