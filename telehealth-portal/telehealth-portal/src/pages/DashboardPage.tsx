...
export default function DashboardPage() {
  const { cases, casesLoading, casesError, refreshCases } = useCases();
  ...
  if (casesLoading && cases.length === 0) {
    return (
      <div style={{ padding: 48, textAlign: "center", color: "#64748b" }}>
        <p style={{ margin: 0, fontSize: 15 }}>Loading cases from API…</p>
      </div>
    );
  }
  ...
  if (casesError && cases.length === 0) {
    return (
      <div style={{ padding: 48, maxWidth: 520, margin: "0 auto" }}>
        <p style={{ color: "#dc2626", marginBottom: 8 }}>{casesError}</p>
        <p style={{ color: "#64748b", fontSize: 13, marginBottom: 16 }}>
          Vercel: set environment variable{" "}
          <code style={{ background: "#f1f5f9", padding: "2px 6px" }}>VITE_API_URL</code> to your
          public backend URL (e.g. https://…onrender.com), then redeploy.
        </p>
        <button
          type="button"
