/**
 * Maps FastAPI /cases/patient-cases contract → telehealth PatientCase (UI model).
 */
...
/** Parse "3 days", "2 weeks", "0" → integer days for display */
...
/**
 * Merge API case into the telehealth PatientCase shape used by existing pages.
 */
export function mapApiCaseToPatientCase(api: ApiPatientCase): ExtendedPatientCase {
  ...
  const base: ExtendedPatientCase = {
    caseId: api.caseId,
    createdAt: submitted,
    updatedAt: submitted,
    ...
    phoneNumber: "—",
    countryCode: api.country?.length === 2 ? api.country : "—",
    country: api.country,
    ...
    doctorReviewed: hasServerCompletion || decision === DoctorDecisionStatus.IN_REVIEW ? 1 : 0,
    doctorReviewedAt: hasServerCompletion || decision === DoctorDecisionStatus.IN_REVIEW ? submitted : null,
