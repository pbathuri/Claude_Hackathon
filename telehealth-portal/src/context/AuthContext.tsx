...
// Sign-in against GET /doctors (license_number on list + detail for email).

import { createContext, useContext, useState, ReactNode } from "react";
import { CountryTier, Doctor } from "../types";
...
function mapApiToDoctor(
  row: Awaited<ReturnType<typeof getDoctor>> | Awaited<ReturnType<typeof getDoctors>>[number],
  email: string
): Doctor {
  const verified =
    ...
    try {
      ...
      if (!match) {
        setState((s) => ({
          ...
          isLoading: false,
          error:
            "License not found or not verified. Use a license registered in the hospital API (see seed data).",
        }));
        return false;
      }
      ...
      setState((s) => ({
        ...
        error:
          e instanceof Error
            ? e.message
            : "Could not reach doctor directory. Is the API running?",
      }));
      return false;
    }
  };
